...
(async function main() {
    try{
        ...
        await page.goto('https://www.aldi.co.uk/c/groceries/fresh-food');
        ...
        for(let i = 0; i < Cat_container.length; i++) {
            ...
            const Folder_cat = await page.$eval('h1.category-name', h1 => h1.innerText.replace(/ *\([^)]*\) */g, "").trim());
        ...
        async function aisle_slct() {
            ...
            for(let i = 0 ; i < Aisle_check_btn.length; i++) {
                ...
                //create a new file 
                const File_name = await Aisle_check_btn[i].$eval('label', label => label.innerText.replace(/ *\([^)]*\) */g, "").trim());
