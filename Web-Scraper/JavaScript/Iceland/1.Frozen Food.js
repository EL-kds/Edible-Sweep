...
(async function main(){
    try {
        ...
        await page.goto('https://www.iceland.co.uk/frozen');
        

        await page.waitForSelector('div.search-suggestions-content');
        const food_page = await page.$('div.search-suggestions-content');
        var Product_btn = await food_page.$$eval('a', a => a.map((elm) => elm.href));
        ...
        async function Ailse_select() {
            ...
            if (await page.$('[id="category-level-1"]')){
                ...
                for (let i = 0; i < 1; i++){ // need to change  <<<<<<<<<<<<<<<<<<<<<<<<<<<<
