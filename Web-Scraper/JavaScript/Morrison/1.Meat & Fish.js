...
(async function main() {
    try{
        ...
        await page.goto('https://groceries.morrisons.com/browse/meat-fish-179549');
        ...
        for (var i = 0; i < Cat_container.length - 5; i++) { // -5 to remove some unwated element 
        ...
        async function aisle_slct() {
            ...
            if(await page.$('div.grocery-section.level-0')) {
                ...
                File_name = await Aisle_page.$$eval('li > a', a => a.map((elm) => elm.innerText.replace('/ ','')));
