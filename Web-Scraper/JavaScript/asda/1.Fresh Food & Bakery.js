...
(async function main() {
    try {
        ...
        await page.goto('https://groceries.asda.com/cat/fresh-food-bakery/103099');
        

        //selecting Product button
        await page.waitForSelector('ul.taxonomy-explore__list'); //wait for page to load element
        const Category_page = await page.$('ul.taxonomy-explore__list');
        const Cat_btn = await Category_page.$$eval('a', a => a.map((elm) => elm.href)); // getting href inner element link from ul class
        
        for(let i = 0; i < Cat_btn.length; i++){
            await page.goto(Cat_btn[i]);
